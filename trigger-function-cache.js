...
async function triggerFunctionCacheRefresh() {
  ...
  const DB_URI = 'mongodb://admin:123456@localhost:27017/laf?authSource=admin';
  ...
  try {
    ...
    const functions = await functionsCollection.find({ appid: 'test-app' }).toArray();
  ...
  // 方法3: 尝试通过服务器API触发缓存刷新
  console.log('\n📋 方法3: 尝试通过服务器API操作');

  try {
    // 尝试获取应用信息
    const appsResponse = await axios.get('http://localhost:3000/v1/applications', {
      timeout: 5000,
      validateStatus: () => true // 接受所有状态码
    });

    console.log(`📡 服务器响应状态: ${appsResponse.status}`);

    if (appsResponse.status === 401) {
      console.log('🔐 需要认证 - 这是正常的');
    } else if (appsResponse.status === 200) {
      console.log('✅ 服务器API可用');
    }

  } catch (error) {
    console.log(`⚠️  服务器API请求失败: ${error.message}`);
  }
