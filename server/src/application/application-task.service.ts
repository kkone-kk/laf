...
export class ApplicationTaskService {
  ...
  async tick() {
    ...
    this.handleCreatedPhase().catch((err) => {
      this.logger.error(err)
    })
  ...
  async handleStartingPhase() {
    ...
    this.logger.log(`handleStartingPhase matched app ${appid}, locked it`)

    // update application phase to `Started`
    await db.collection<Application>('Application').updateOne(
      { _id: app._id, phase: ApplicationPhase.Starting },
      {
        $set: {
    ...
    this.logger.log('app phase updated to `Started`: ' + app.appid)
  }
