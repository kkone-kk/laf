...
export class FunctionTemplateService {
  ...
  async createFunctionTemplate(
    ...
    try {
      ...
      await this.db
        ...
        .insertMany(functionTemplateItems)
      ...
      this.logger.error('Error creating function template:', error)
      throw error
    }
  }
  ...
  async updateFunctionTemplate(
    ...
    try {
      ...
      if (res.lastErrorObject.updatedExisting) {
        ...
        await this.db
          .collection<FunctionTemplateItem>('FunctionTemplateItem')
          .deleteMany({ templateId: templateId })
        await this.db
          .collection<FunctionTemplateItem>('FunctionTemplateItem')
          .insertMany(functionTemplateItems)
      }
      ...
      this.logger.error('Error updating function template:', error)
      throw error
    }
  }
  ...
  async useFunctionTemplate(
    ...
    try {
      ...
      const itemsToInsert = await this.db
        .collection<CloudFunction>('CloudFunction')
        .find({ appid: appid, name: { $in: namesToSearch } })
        .toArray()
      ...
      this.logger.error('Error using function template:', error)
      throw error
    }
  }
  ...
  async deleteFunctionTemplate(templateId: ObjectId, userid: ObjectId) {
    try {
      ...
      await this.db
        .collection<FunctionTemplate>('FunctionTemplate')
        .deleteOne({ _id: templateId, uid: userid })
      ...
      this.logger.error('Error deleting function template:', error)
      throw error
    }
  }
  ...
  async starFunctionTemplate(templateId: ObjectId, userid: ObjectId) {
    try {
      ...
      if (found) {
        await this.db
          .collection<FunctionTemplate>('FunctionTemplate')
          .updateOne({ _id: templateId }, { $inc: { star: -1 } })

        await this.db
          .collection<FunctionTemplateStarRelation>(
            'FunctionTemplateStarRelation',
          )
          .deleteOne({ uid: userid, templateId: templateId })
      ...
      this.logger.error('Error starring function template:', error)
      throw error
    }
  }
